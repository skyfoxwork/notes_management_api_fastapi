...
async def get_analyze_notes(notes: list):
    ...
    df = pd.DataFrame(
        [{"id": note.id, "content": note.content} for note in notes]
    )
    ...
    words = [
        word for word in word_tokenize(all_words)
        if word.isalnum() and word not in STOPWORDS
    ]
    most_common_words = Counter(words).most_common(5)

    # Top 3 Long and Short Notes
    longest_notes = df.nlargest(
        3, "word_count"
    )[["id", "word_count"]].to_dict(orient="records")
    shortest_notes = df.nsmallest(
        3, "word_count"
    )[["id", "word_count"]].to_dict(orient="records")
