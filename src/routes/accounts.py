...
async def register_user(
    ...
    """
    db_result_user = await db.execute(select(UserModel).filter_by(email=user_data.email))
    existing_user = db_result_user.scalars().first()
    ...
    db_result_group = await db.execute(select(UserGroupModel).filter_by(name=UserGroupEnum.USER))
    user_group = db_result_group.scalars().first()
...
async def login_user(
    ...
    """
    db_result_user = await db.execute(select(UserModel).filter_by(email=login_data.email))
    user = cast(UserModel, db_result_user.scalars().first())
...
async def refresh_access_token(
    ...
    db_result_user = await db.execute(select(UserModel).filter_by(id=user_id))
